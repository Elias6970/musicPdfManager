...
from frontend.pyqt.app.config.constants import EDIT_IMG_PATH, TRASH_IMG_PATH
...
class StatusConsoleItemWithTwoButtons(QtWidgets.QFrame):
    edit_signal = QtCore.pyqtSignal(str)
    delete_signal = QtCore.pyqtSignal(str)
    def __init__(self, name:str, tool_tip:str,parent=None) -> None:
        ...
        self.edit_btn.clicked.connect(lambda: self.edit_signal.emit(name))
        self.delete_btn.clicked.connect(lambda: self.delete_signal.emit(name))
