...
def test_on_fetch_finished_success(preview_client, mock_reply):
    ...
    mock_reply.rawHeader.return_value = b"5"
    ...
    # Execute
    preview_client._on_fetch_finished(mock_reply, page_num=2)
    
    # Assert
    loaded_slot.assert_called_once_with(2, b"fake_image_bytes", 5)
...
def test_on_fetch_finished_success_no_header_defaults_to_one(preview_client, mock_reply):
    ...
    mock_reply.rawHeader.return_value = b""  # Empty header
    
    loaded_slot = MagicMock()
    preview_client.preview_loaded.connect(loaded_slot)
    
    preview_client._on_fetch_finished(mock_reply, page_num=1)
    
    loaded_slot.assert_called_once_with(1, b"fake_image_bytes", 1)
