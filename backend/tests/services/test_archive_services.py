...
def test_get_all_piece_std_names(session: Session):
    p1 = Piece(cod=1, name="Piece 1", std_name="1-Piece 1", archive_id=1, digitalized=False, handwrited=False, parted=False)
    p2 = Piece(cod=2, name="Piece 2", std_name="2-Piece 2", archive_id=1, digitalized=True, handwrited=False, parted=False)
...
def test_delete_piece_from_archive_success(session: Session, file_manager: MagicMock):
    p1 = Piece(cod=1, name="ToDelete", std_name="1-ToDelete", archive_id=1, digitalized=False, handwrited=False, parted=False)
    session.add(p1)
    session.commit()
...
def test_add_files_to_existing_piece_success(session: Session, file_manager: MagicMock):
    p1 = Piece(cod=5, name="Existing", std_name="5-Existing", archive_id=1, digitalized=False, handwrited=False, parted=False)
    session.add(p1)
    session.commit()
...
def test_add_files_to_existing_piece_copy_fail(session: Session, file_manager: MagicMock):
    p1 = Piece(cod=6, name="Existing2", std_name="6-Existing2", archive_id=1, digitalized=False, handwrited=False, parted=False)
    session.add(p1)
    session.commit()
